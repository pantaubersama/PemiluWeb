  ...
  watch: {
    $route: {
      ...
      async handler($route) {
        ...
        if (realCountId != null) {
          this.getForm({ realCountId, type: 'dpr' })
        }
        if (province != null) {
          this.listDapils({ provinceCode: province, tingkat: 'dpr' })
        }
        if (dapil != null) {
          this.listCandidates({ dapilId: dapil, tingkat: 'dpr' })
            .then(() => {
              this.getCandidatesSummary({ dapilId: dapil, level: 0, realCountId })
            })
        }
      }
    },
